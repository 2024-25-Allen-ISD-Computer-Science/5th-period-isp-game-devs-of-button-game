...
func _ready():
	...
	if collider and collider.has_signal("area_entered"):
			collider.connect("area_entered", Callable(self, "on_area_entered"))
	#connect("body_entered", Callable(self, "_body_entered"))
	if connect_portal and connect_portal.has_signal("portal_body_entered"):
		connect("portal_body_entered", Callable(self, "_body_entered"))
...
func _handle_crouch(delta) -> void:
	...
	if TranslateYIfPossible != 0.0:
		...
		%Head.position.y = clamp(%Head.position.y, -CrouchTranslate, 0)
	
	%Head.position.y = move_toward(%Head.position.y, -CrouchTranslate if is_crouched else 0, 0.0 * delta)
...
#SURFING CODE/ like not allowing the player to go into walls, honestly I don't think we'll use this n our game but better safe than sorry :)))
...
func on_area_entered(area: Area3D):
	if area.is_in_group("ElectricOrbs"):
		print("An area entered: ", area)

		emit_signal("orb_collected")
		area.queue_free()
		#if $orbsound and $orbsound is AudioStreamPlayer:
			#$orbsound.stop()
			#$orbsound.play()
		#else:
			#print("orbsound node is missing or invalid!")
...
func _body_entered(body: Node3D) -> void:
	if body.name == "Player" and target_scene:
		print("Teleporting to Buttonroom scene...")
		get_tree().change_scene("res://Buttonroom(1).tscn")
			#var destination = connect_portal.global_transform.origin
			#body.global_transform.origin = destination
		#else:
			#print("Target scene is not assigned!")
